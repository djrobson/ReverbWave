 ...
 * It handles audio processing, parameter management, and DSP operations
 * including:
 ...
 *
 * Implements AudioProcessor for audio handling and ValueTreeState::Listener for
 * parameter updates
 */
class CustomReverbAudioProcessor
...
public:
  ...
  void processCrossover(float leftIn, float rightIn, float &leftLow,
                        float &leftHigh, float &rightLow, float &rightHigh);

  /** handle delay differently for high frequencies */
  void processHighFreqDelay(float leftIn, float rightIn, float &leftOut,
                            float &rightOut);
...
private:
  ...
  struct CustomReverbParameters {
    float roomSize =
        0.5f; // Controls the size of the simulated room (0.0=small, 1.0=large)
    float damping =
        0.5f; // Controls high frequency absorption (0.0=bright, 1.0=dark)
    float wetLevel =
        0.33f; // Amount of processed signal in output (0.0=dry, 1.0=wet)
    float dryLevel =
        0.4f; // Amount of unprocessed signal in output (0.0=none, 1.0=full)
    float width = 1.0f; // Stereo width enhancement (0.0=mono, 1.0=wide)
    float freezeMode =
        0.0f; // Infinite reverb tail when 1.0 (0.0=normal, 1.0=freeze)
    float highFreqDelay = 0.3f; // Separate delay for high frequencies (0.0=same
                                // as low, 1.0=max delay)
    float highFreqDelayMix =
        0.3f; // Mix of high frequency delay (0.0=none, 1.0=full)
    float crossover =
        0.5f; // Frequency split point between low/high bands (0.5≈1000Hz)
    float harmDetuneAmount = 0.0f; // Stereo enhancement via harmonic detuning
  ...
  /** Creates the parameter layout and ranges for the
   * AudioProcessorValueTreeState */
  juce::AudioProcessorValueTreeState::ParameterLayout createParameters();

  /** Updates the reverb parameters based on current values in the parameter
   * tree */
  ...
  std::vector<float>
      oddHarmonicBufferL; // Buffer for odd harmonics (left channel)
  std::vector<float>
      evenHarmonicBufferR; // Buffer for even harmonics (right channel)
  ...
  bool nextFFTBlockReady =
      false; // Flag indicating when FFT block is ready to process
