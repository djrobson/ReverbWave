...
SpectrumAnalyzerComponent::SpectrumAnalyzerComponent(
    CustomReverbAudioProcessor &processor)
    : processorRef(processor), fft(processor.fftOrder),
...
void SpectrumAnalyzerComponent::paint(juce::Graphics &g) {
  ...
  if (sampleRate > 0) {
    const char *freqLabels[] = {"20", "50", "100", "200", "500",
                                "1k", "2k", "5k",  "10k", "20k"};
    const float freqValues[] = {20.0f,   50.0f,   100.0f,  200.0f,   500.0f,
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      if (x >= 0 && x < width) {
        g.drawText(freqLabels[i], static_cast<int>(x) - 10, (int)height - 20,
  ...
  } else if (animationMode == 1) // Bar mode
  {
    ...
    for (int i = 0; i < numBars; ++i) {
      ...
      float hue = colorScheme == 0 ? 0.6f - 0.2f * wavePoints[i]
                                   : // Blue scheme
                      colorScheme == 1 ? 0.8f - 0.2f * wavePoints[i]
                                       :           // Purple scheme
    ...
    for (int i = 0; i < numBars; ++i) {
      ...
      if (level > 0.05f) {
        ...
        for (int j = 0; j < numParticles; ++j) {
          float particleSize =
              (1.0f + 4.0f * level) * (0.5f + 0.5f * random.nextFloat());

          // Particle position: x based on frequency, y based on amplitude with
          // randomness
          float x = i * barWidth + random.nextFloat() * barWidth;
          float y =
              height - height * level * (0.5f + 0.5f * random.nextFloat());

          // Color based on frequency and scheme
          float hue =
              colorScheme == 0 ? 0.6f - 0.2f * (static_cast<float>(i) / numBars)
                               : // Blue scheme
                  colorScheme == 1
                  ? 0.8f - 0.3f * (static_cast<float>(i) / numBars)
                  : // Purple scheme
                  0.3f -
                      0.2f * (static_cast<float>(i) / numBars); // Green scheme

          juce::Colour particleColor =
              juce::Colour::fromHSV(hue, 0.8f, 0.9f, 0.7f);

          g.setColour(particleColor);
          g.fillEllipse(x - particleSize / 2, y - particleSize / 2,
...
void SpectrumAnalyzerComponent::updateSpectrum(const float *spectrumData,
                                               int numBins) {
...
void SpectrumAnalyzerComponent::timerCallback() {
  ...
  for (int i = 0; i < spectrumValues.size(); ++i) {
    ...
    spectrumValues[i] = previousSpectrumValues[i] +
                        smoothingCoefficient * (targetSpectrumValues[i] -
                                                previousSpectrumValues[i]);
  }
...
CustomReverbAudioProcessorEditor::CustomReverbAudioProcessorEditor(
    CustomReverbAudioProcessor &p)
    : AudioProcessorEditor(&p), audioProcessor(p), spectrumAnalyzer(p) {
  // Set up custom look and feel
  customLookAndFeel.setColour(juce::Slider::thumbColourId,
                              juce::Colour(100, 180, 240));
  customLookAndFeel.setColour(juce::Slider::trackColourId,
                              juce::Colour(80, 90, 100));
  customLookAndFeel.setColour(juce::Slider::backgroundColourId,
                              juce::Colour(40, 50, 60));
  ...
  dampingAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(apvts, "damping",
                                                               dampingSlider));
  ...
  widthAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(apvts, "width",
                                                               widthSlider));

  // High Frequency Delay Slider
  highFreqDelaySlider.setSliderStyle(juce::Slider::RotaryVerticalDrag);
  highFreqDelaySlider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 80,
                                      20);
  ...
  highFreqDelayAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(
          apvts, "highFreqDelay", highFreqDelaySlider));

  // High Frequency Mix Slider
  highFreqMixSlider.setSliderStyle(juce::Slider::RotaryVerticalDrag);
  highFreqMixSlider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 80, 20);
  highFreqMixSlider.setLookAndFeel(&customLookAndFeel);
  addAndMakeVisible(highFreqMixSlider);

  highFreqMixLabel.setText("HF Mix", juce::dontSendNotification);
  highFreqMixLabel.setJustificationType(juce::Justification::centred);
  addAndMakeVisible(highFreqMixLabel);

  highFreqMixAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(
          apvts, "highFreqMix", highFreqMixSlider));
  ...
  crossoverAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(
          apvts, "crossoverFreq", crossoverSlider));

  // Harmonic Detune Amount Slider
  harmDetuneAmountSlider.setSliderStyle(juce::Slider::RotaryVerticalDrag);
  harmDetuneAmountSlider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 80,
                                         20);
  ...
  harmDetuneAmountAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(
          apvts, "harmonicDetune", harmDetuneAmountSlider));
  // Freeze Mode Button
  freezeModeButton.setButtonText("Freeze");
  addAndMakeVisible(freezeModeButton);

  freezeModeAttachment.reset(
...
CustomReverbAudioProcessorEditor::~CustomReverbAudioProcessorEditor() {
  ...
  highFreqMixSlider.setLookAndFeel(nullptr);
...
void CustomReverbAudioProcessorEditor::paint(juce::Graphics &g) {
  ...
  juce::ColourGradient gradient(juce::Colour(15, 25, 35), 0, 0,
                                juce::Colour(40, 50, 60), (float)getWidth(),
                                (float)getHeight(), false);
  g.setGradientFill(gradient);
  g.fillRect(getLocalBounds());

  // Draw title
  juce::FontOptions fOptions(24.0f, juce::Font::bold);
  g.setFont(juce::Font(fOptions));
  g.setColour(juce::Colours::white);
  g.drawText("Custom Reverb Plugin", 20, 15, getWidth() - 40, 30,
...
void CustomReverbAudioProcessorEditor::resized() {
  ...
  highFreqDelayLabel.setBounds(highFreqDelaySlider.getX(),
                               highFreqDelaySlider.getY() - 15,
                               highFreqDelaySlider.getWidth(), 20);

  highFreqMixSlider.setBounds(row2.removeFromLeft(sliderWidth).reduced(10));
  highFreqMixLabel.setBounds(highFreqMixSlider.getX(),
                             highFreqMixSlider.getY() - 15,
                             highFreqMixSlider.getWidth(), 20);
  ...
  harmDetuneAmountSlider.setBounds(
      row2.removeFromLeft(sliderWidth).reduced(10));
  harmDetuneAmountLabel.setBounds(harmDetuneAmountSlider.getX(),
                                  harmDetuneAmountSlider.getY() - 15,
...
void CustomReverbAudioProcessorEditor::loadPreset(int presetIndex) {
  ...
  case 0: // Small Room
    ...
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.4f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.0f);
    break;
  ...
  case 1: // Medium Room
    ...
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.5f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.0f);
    break;
  ...
  case 2: // Large Hall
    ...
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.3f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.0f);
    break;
  ...
  case 3: // Cathedral
    ...
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.2f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.0f);
    break;
  ...
  case 4: // Special FX
    ...
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.7f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.0f);
    break;
  ...
  case 5: // Bright Chamber
    ...
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.8f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.0f);
    break;
  ...
  case 7: // Harmonic Detuner
    ...
    apvts.getParameter("freezeMode")->setValueNotifyingHost(0.0f);
    apvts.getParameter("highFreqDelay")->setValueNotifyingHost(0.3f);
    apvts.getParameter("crossoverFreq")->setValueNotifyingHost(0.6f);
    apvts.getParameter("harmonicDetune")->setValueNotifyingHost(0.7f);
    break;
  }
}
